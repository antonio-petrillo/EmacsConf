...
As [[https://protesilaos.com/emacs/dotemacs][Protesilaos]] explains on the website, set the *garbage collector threshold*, for the whole emacs instance life, to the highest possible it's a bad idea, although it improve the startup on the long run the performance worsen.
...
Here another nice /hack/ to avoid the initial whitescreen, before my selected theme is loaded in laters steps, on opening emacs.
#+begin_src emacs-lisp :tangle early-init.el
...
* Init
The following step is to load the good old ~init.el~, here it is possible to enable or not the various piece of the configuration, every piece can (better say should, since I didn't made any test) be omitted with few exception, every piece can be omitted with few exceptions.
...
*** Enabling modes
+ /recentf/, completion systems suggests recently opened files
...
** Module ~init-evil.el~
*** Escape advice
Required to avoid strange behaviour when pressing src_emacs-lisp{(kbd  "<escape>)}
...
I define two leader:
+ =SPC= for global shortcuts
+ =m= as local leader for specific modes
#+begin_src emacs-lisp :tangle "lisp/init-evil.el" :mkdirp yes
(use-package general
  :after evil
  :init
...
*** EVIL setup
I learned first vim bindings and only after I switched to *Emacs* for this reason I preferred to stick with the modal editor approach.
Even tough for a couple of months I sticked to the *default* bindings and learned (not in deepth) them, and honestly they are not that bad as everybody says, so I mix the twos, doing so I've lost some vim bindings but I don't use them anyway.
#+begin_src emacs-lisp :tangle "lisp/init-evil.el" :mkdirp yes
(use-package evil
  :init
...
** Module ~init-org.el~
*** Header 
To use, in a proper way, lambda functions.
...
**** Configuration
Use *org roam v2*.
Add keybinding to interact with it.
It is loaded only if the *org-directory* is defined.
#+begin_src emacs-lisp :tangle "lisp/init-org.el" :mkdirp yes
...
Highlight matching parentheses and setting up src_emacs-lisp{electric-pair}, when enabled closing bracket are inserted automatically a closing bracket is inserted automatically
#+begin_src emacs-lisp :tangle "lisp/init-editing.el" :mkdirp yes
(show-paren-mode t)
...
A neat example, which is more of a memorandum for me, is the following:
...
As usual, there are many packages to accomplish this, personally I like src_emacs-lisp{apheleia}, by the same creator of src_emacs-lisp{straight.el}.
#+begin_src emacs-lisp :tangle "lisp/init-editing.el" :mkdirp yes
...
**** Side note
This package provide come with some auto formatter preconfigured, but not for every language.
I need to provide one, like =cljfmt= for clojure.
*** End
...
A vscode like, popup to select between completion.
#+begin_src emacs-lisp :tangle "lisp/init-completion.el" :mkdirp yes
(use-package company
  :init
...
+ src_emacs-lisp{delete-by-moving-to-trash}, on delete move to trash instead of perform a src_bash{rm} on candidate
+ src_emacs-lisp{dired-listing-switches}, src_emacs-lisp{ls} switches to use
+ src_emacs-lisp{dired-dwim-target}, *do what I mean* on target
...
*** Hide hidden files
I like the src_bash{ls -a} switch but often I don't need all that information, I prefer to hide all the dot file and show them only when I need them.
#+begin_src emacs-lisp :tangle "lisp/init-dired.el" :mkdirp yes
...
Recently I've read [[https://karthinks.com/software/fifteen-ways-to-use-embark/][fifteen ways to use embark]] and it is a game changing, it require a little bit to stick to it but the gain is incredible.
*** Install embark
